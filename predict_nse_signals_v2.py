...
def generate_predictions(model, scaler, encoder, selected_features, df):
    ...
    # Model name -- derived from the loaded artifact so LightGBM models are
    # stamped correctly. Must keep containing 'V2' (the delete/summary/backfill
    # queries all filter model_name LIKE '%V2%').
    base = model.base_model if hasattr(model, 'base_model') else model
    predictions['model_name'] = ('LightGBM_V2' if type(base).__name__ == 'LGBMClassifier'
                                 else 'GradientBoosting_V2_Calibrated')
...
def save_summary(conn, predictions):
    ...
    """, (
        ...
        total_predictions, f"Generated by {predictions['model_name'].iloc[0]} model"
    ))
